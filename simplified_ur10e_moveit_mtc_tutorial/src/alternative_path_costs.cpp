...
int main(int argc, char** argv) {
    ...
    {
      // std::vector<std::string> joints = {"shoulder_pan_joint"};

      auto connect{ std::make_unique<stages::Connect>(
           "path length", stages::Connect::GroupPlannerVector{ { "ur_arm_with_grip", pipeline } }) };
    ...
    {
      auto connect{ std::make_unique<stages::Connect>(
           "trajectory duration", stages::Connect::GroupPlannerVector{ { "ur_arm_with_grip", pipeline } }) };
    ...
    {
      auto connect{ std::make_unique<stages::Connect>(
           "eef motion", stages::Connect::GroupPlannerVector{ { "ur_arm_with_grip", pipeline } }) };
      connect->setCostTerm(std::make_unique<cost::LinkMotion>("flange_base")); // Distance traveled by the link in meters
      alternatives->add(std::move(connect));
    ...
    {
      auto connect{ std::make_unique<stages::Connect>(
           "elbow motion", stages::Connect::GroupPlannerVector{ { "ur_arm_with_grip", pipeline } }) };
      connect->setCostTerm(std::make_unique<cost::LinkMotion>("wrist_1_link"));
      alternatives->add(std::move(connect));
    ...
    goal_scene->getCurrentStateNonConst().setToDefaultValues(robot_state.getJointModelGroup("ur_arm"), "pose_random2");
    std::cout << "Created goal scene with 'pose_random1' position" << std::endl;
    ...
    try {
      t.plan(2); // The 0 parameter means it will generate as many solutions as possible
