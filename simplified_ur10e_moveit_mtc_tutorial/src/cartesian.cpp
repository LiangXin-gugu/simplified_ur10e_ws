...
Task createTask(const rclcpp::Node::SharedPtr& node) {
  ...
  const std::string arm = "ur_arm";
  // const std::string arm_with_gripper = "ur_arm_with_grip";
  ...
  // Set the initial state of the robot
  {
    ...
    state.setToDefaultValues(state.getJointModelGroup(arm), "pose_official");
  ...
  {
    RCLCPP_INFO(node->get_logger(), "Creating stage: Move 0.5m in +x direction");
        
    // Create a MoveRelative stage using Cartesian interpolation
    auto stage = std::make_unique<stages::MoveRelative>("x +0.5", cartesian_interpolation);
        
    // Specify which group of joints to move 
    stage->setGroup(arm);
    
    // Set the Inverse Kinematic frame to the end-effector
    stage->setIKFrame("flange_base");
    ...
    // Set the x component to 0.5 meters (move 50 cm in the x direction)
  ...
  {
    RCLCPP_INFO(node->get_logger(), "Creating stage: Move 0.5m in -y direction");
       
    // Similar to the previous stage, but moving in the y direction
    auto stage = std::make_unique<stages::MoveRelative>("y -0.5", cartesian_interpolation);
    ...
    // Set the y component to -0.5 meters (move 50 cm in the negative y direction)
  ...
  {
    RCLCPP_INFO(node->get_logger(), "Creating stage: Rotate -90 degrees around z-axis");
        
    auto stage = std::make_unique<stages::MoveRelative>("rz -90°", cartesian_interpolation);
    stage->setGroup(arm);
    stage->setIKFrame("flange_base");
    ...
    // Set the angular z component to -pi/10 radians (-90 degrees)
  ...
  // Stage 4: Move specific joints by specified angles
  {
    ...
    std::map<std::string, double> offsets = {
      { "shoulder_pan_joint", M_PI / 8. },  // Rotate this joint by 22.5 degrees (pi/12 radians)
      { "wrist_2_joint", -M_PI / 8. }   // Rotate this joint by -22.5 degrees
    };
