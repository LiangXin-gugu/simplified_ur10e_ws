...
int main(int argc, char** argv) {
  ...
  [[maybe_unused]] bool found =
      robot_state.setToDefaultValues(robot_state.getJointModelGroup("ur_arm"), "pose_official");
  assert(found);
  RCLCPP_INFO(logger, "Arm set to 'pose_official' position");
  ...
  co.primitives[0].dimensions[0] = 0.5;  
  ...
  // Set the target pose ://end effector relative to world(or base link?) 
  ik->setTargetPose(Eigen::Translation3d(-0.2, 0.2, 0.5) * Eigen::AngleAxisd(M_PI_4, Eigen::Vector3d::UnitY()));
