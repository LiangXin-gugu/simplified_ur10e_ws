...
#include <simplified_ur10e_moveit_mtc_tutorial/pick_place_task.h>
...
namespace simplified_ur10e_moveit_mtc_tutorial {
...
bool PickPlaceTask::init(const rclcpp::Node::SharedPtr& node, const pick_place_demo::Params& params) {
	...
	Stage* initial_state_ptr = nullptr;
	{
		...
		stage->properties().set("marker_ns", "open_gripper");
		...
		stage->properties().set("trajectory_execution_info",
		                        TrajectoryExecutionInfo().set__controller_names(params.grip_controller_name));
	 ...
	 *****************************************************/
	{
		auto stage = std::make_unique<stages::MoveTo>("move to ready", ompl_planner_arm);
		...
		stage->properties().set("trajectory_execution_info",
		                        TrajectoryExecutionInfo().set__controller_names(params.arm_controller_name));
		t.add(std::move(stage));
	}
	...
	// This involves calculating and executing a valid trajectory using the ompl_planner_arm.
	{
		...
		stage->properties().set("marker_ns", "move_to_pick");
		stage->properties().set("trajectory_execution_info",
		                        TrajectoryExecutionInfo().set__controller_names(params.arm_controller_name));
		t.add(std::move(stage));
	}
	...
	Stage* pick_stage_ptr = nullptr; // Store a reference to the entire pick/grasp stage of the task.
	{
		...
		grasp->properties().set("marker_ns", "pick object");
  ...
  ---- *               Generate Grasp Pose                *
		 ***************************************************/
		{
			...
			stage->setEndEffector(params.eef_name);//necessary
			stage->setPreGraspPose(params.gripper_open_pose);//necessary
			...
			wrapper->setMaxIKSolutions(2);  // limit number of solutions to reduce computational overhead
			wrapper->setMinSolutionDistance(0.5); // Solutions must be distinct from each other
			// define virtual frame to reach the target_pose. To make the virtual frame consistant to object frame
  ...
  .... *               Forbid collision (object, surface)*
	 ...
	 *****************************************************/
	{
		...
		stage->properties().set("marker_ns", "move_to_place");
		stage->properties().set("trajectory_execution_info",
		                        TrajectoryExecutionInfo().set__controller_names(params.arm_controller_name));
		t.add(std::move(stage));
	}
  ...
  ---- *          Retreat Motion                            *
		 *****************************************************/
		{
			...
			stage->setGroup(params.arm_group_name);
			stage->setIKFrame(params.gripper_frame);
			stage->setMinMaxDistance(params.retreat_object_min_dist, params.retreat_object_max_dist);
			stage->properties().set("trajectory_execution_info",
		                        TrajectoryExecutionInfo().set__controller_names(params.arm_controller_name));

			// Set retreat direction for retreating gripper
			geometry_msgs::msg::Vector3Stamped vec;
			vec.header.frame_id = params.gripper_frame;
			vec.vector.z = -1.0;
			stage->setDirection(vec);
