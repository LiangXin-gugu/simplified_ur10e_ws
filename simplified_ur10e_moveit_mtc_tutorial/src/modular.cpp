...
std::unique_ptr<SerialContainer> createModule(const std::string& group) {
  ...
  {
    auto stage = std::make_unique<stages::MoveRelative>("x +0.5", cartesian);
    ...
    RCLCPP_INFO(rclcpp::get_logger("modular_demo"), "Added stage: Move 50 cm in +X direction");
  }

  // Stage 2: Move 2 cm in the negative Y direction
  {
    auto stage = std::make_unique<stages::MoveRelative>("y -0.5", cartesian);
    ...
    stage->setDirection(direction);
    c->insert(std::move(stage));
    RCLCPP_INFO(rclcpp::get_logger("modular_demo"), "Added stage: Move 50 cm in -Y direction");
  }
  ...
  // Stage 3: Rotate -18 degrees around the Z axis
  {
    ...
    twist.twist.angular.z = -M_PI / 4.; // 45 degrees in radians
    stage->setDirection(twist);
    c->insert(std::move(stage));
    RCLCPP_INFO(rclcpp::get_logger("modular_demo"), "Added stage: Rotate -45 degrees around Z axis");
  }

  // Stage 4: Move to the "ready" position
  {
    auto stage = std::make_unique<stages::MoveTo>("moveTo pose_official", joint_interpolation);
    stage->properties().configureInitFrom(Stage::PARENT);
    stage->setGoal("pose_official");
    c->insert(std::move(stage));
    RCLCPP_INFO(rclcpp::get_logger("modular_demo"), "Added stage: Move to 'pose_official' position");
  }
...
Task createTask(const rclcpp::Node::SharedPtr& node) {
  ...
  const std::string group = "ur_arm";

  // Add a stage to move to the "pose_official" position
  {
    auto stage = std::make_unique<stages::MoveTo>("move to pose_official", std::make_shared<solvers::JointInterpolationPlanner>());
    stage->setGroup(group);
    stage->setGoal("pose_official");
    t.add(std::move(stage));
    RCLCPP_INFO(node->get_logger(), "Added stage: Move to 'pose_official' position");
  }
