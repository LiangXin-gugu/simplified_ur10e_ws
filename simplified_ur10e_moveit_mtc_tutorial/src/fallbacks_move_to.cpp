...
int main(int argc, char** argv) {
 ...
 // Declare the node parameters
  ...
  robot->getJointModelGroup("ur_arm")->getVariableDefaultPositions("pose_official", target_state);
  target_state["shoulder_pan_joint"] = +TAU / 8;
  RCLCPP_INFO(logger, "Target state set for 'ur_arm' group");
  ...
  initial_scene->getCurrentStateNonConst().setToDefaultValues(robot->getJointModelGroup("ur_arm"), "pose_official");
  ...
  // First initial state option: 90 degree offset from target goal
  {
    ...
    scene->getCurrentStateNonConst().setVariablePositions({ { "shoulder_pan_joint", -TAU / 8 } });
