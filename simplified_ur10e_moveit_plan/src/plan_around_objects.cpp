...
int main(int argc, char * argv[])
{
  ...
  arm_group_interface.setPlanningTime(3.0);
  ...
  auto const draw_trajectory_tool_path = 
    [&moveit_visual_tools, jmg = arm_group_interface.getRobotModel()->getJointModelGroup("ur_arm")](
  ...
  auto const arm_target_pose = [&node]{
    ...
    msg.pose.position.x = 0.442;//pose_official
    msg.pose.position.y = -0.153;
    msg.pose.position.z = 0.693;
    msg.pose.orientation.x = -0.026;
    msg.pose.orientation.y = 0.917;
    msg.pose.orientation.z = 0.059;
    msg.pose.orientation.w = 0.394;

    // msg.pose.position.x = -0.0012675;//pose_stretch
    // msg.pose.position.y = -0.3407;
    // msg.pose.position.z = 1.4848;
    // msg.pose.orientation.x = 0.70711;
    // msg.pose.orientation.y = -0.00031548;
    // msg.pose.orientation.z = 0.00032614;
    // msg.pose.orientation.w = 0.70711;
    return msg;
  }();
  ...
  auto const collision_object = [frame_id = arm_group_interface.getPlanningFrame(), &node, &logger] {
    ...
    primitive.dimensions[primitive.BOX_Z] = 1.00;  // Height

    // Define the pose (position and orientation) of the box
    geometry_msgs::msg::Pose box_pose;
  
    // Set the position of the box center
    box_pose.position.x = -0.2;  // meters in x-direction
    box_pose.position.y = 0.0;   // Centered in y-direction
    box_pose.position.z = 0.50;//0.25;  // meters in z-direction
  ...
  // Create ground_plane for the robot to avoid
  auto const ground_plane = [frame_id = arm_group_interface.getPlanningFrame(), &node, &logger] {
    ...
    box_pose.position.y = 0.0;   // Centered in y-direction
    box_pose.position.z = -0.005;  // meters in z-direction
  ...
  prompt("Press 'next' in the RvizVisualToolsGui window to start plan");
